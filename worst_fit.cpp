...
int main()
{
    ...
    while (is_running)
    {
        ...
        else if (command == "new")
        {
            ...
            if (requested_size == 0)
            {
                cout << "Process actually don't exist or error in creating the process" << endl;
            }
            else
            {
                ...
                for (int i = 0; i < TOTAL; i++)
                {
                    if (memory[i] == 0)
                    {
                        ...
                        int free_block_size = j - i;

                        if (free_block_size >= requested_size && free_block_size > largest_free_block_size)
                        {
                            start_address = i;
                            largest_free_block_size = free_block_size;
                        }
                        i = j;
                    }
                }
